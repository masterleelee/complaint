...
DEFAULT_CONFIG = {
    ...
    # 百度云 OCR（合同图片 OCR 主路，spec: .scratch/contract-ocr-baidu/spec.md v3）。
    # 密钥优先读环境变量 BAIDU_OCR_API_KEY / BAIDU_OCR_SECRET_KEY（不入库、不落代码）；
    # empty_env_fallback=True 且环境变量为空时，回落读本段 api_key/secret_key。
    "baidu_ocr": {
        "enabled": os.getenv("BAIDU_OCR_ENABLED", "1") not in ("0", "false", "no"),
        "api_key": "",
        "secret_key": "",
        "timeout": 15,
        "empty_env_fallback": True,
    },
    "paths": {
