...
七档合同（源自 `1合同种类/` 八份模板，2021/2022 两份正文仅格式差异合并一档）：
...
CONTRACT_TIERS = [
    ...
    {
        # 东城自制培训合同（1合同种类/东城自制培训合同.docx，文件创建 2019-01-29）。
        # 地区网点自制的合并版：一份合同同时涵盖培训 + 代交费用 + 协助报考服务，
        # 与 2019 三份配套（服务/代缴/培训各一份）结构不同。
        # 退费走第六条专属分支（deduction_engine），非「必扣项+违约金」线性模型。
        # 用户拍板（2026-09-02 修订）：违约金 20%（第十一条「总培训费用20%」）、
        # 实操单价采信合同正文第六条（三）「80 元/学时」（C1/C2 统一 80，不沿用
        # 东莞驾培 C1=120/C2=150 标准——地区自制合同以其手写约定为准）、
        # 代交费一次性 490 元；不限年份、不限网点（靠特征句识别）。
        "id": "2019_dongcheng",
        "display_name": "东城自制",
        "kind": "单一培训",
        "penalty_rate": 20,
...
# 东城自制第十一条用「总培训费用20%的违约金」句式，基数同为培训费总额，纳入比对。
_TEXT_PENALTY_PATTERNS = (
    re.compile(r"全部培训费用的(\d{1,2})%"),
    re.compile(r"总培训费用(\d{1,2})%"),
)
